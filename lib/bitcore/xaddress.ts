...
function decode(address: string): XAddressData {
  ...
  // Legacy checksum format used in lotusd v1.0.1 and prior
  // The legacy format incorrectly uses Bitcoin-style varint-encoded length prefixes
  // for the prefix and payload (like CHashWriter SER_GETHASH), rather than simple
  // concatenation. We validate against both formats to maintain backward compatibility
  // with addresses generated by older lotusd versions.
  // Reference: lotusd/src/addresses/xaddress.cpp LegacyHashAddressContents() (lines 38-50)
  ...
  // Validate checksum or legacy checksum
  const validChecksum =
